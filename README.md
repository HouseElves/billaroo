...
- billing behaviour beyond the model operation and the generate-invoice action: monthly billing integration, run-level invoice accumulation, and raw invoice/invoice-line file emission (`build_account_month_invoice` computes one account-month invoice, and the `GenerateInvoiceIntent` chain produces it as an action result, but the monthly run does not yet invoke billing, accumulate invoices, or emit invoice files)
...
The `Invoice` and `InvoiceLine` record contracts, their `build_invoice` / `build_invoice_line` builders, the pure model operation `build_account_month_invoice(...)`, and a `GenerateInvoiceIntent` semantic action now exist: given a state, catalog, account, and month, the model determines which subscriptions are chargeable and builds one reconciled account-month invoice with ordered lines, and the generate-invoice action chain produces that invoice and its lines as an `ActionResult`. But the monthly simulation still does not invoke billing: it generates no invoices during a run, accumulates none across the horizon, and emits no invoice or invoice-line files. The hidden truth ledger, the rest of the billing and payments capability, the PostgreSQL loader, the dbt reconstruction layer, the validation comparison, and all non-cancellation lifecycle transitions are **planned architecture, not current capability**. The roadmap is recorded honestly in the design log; nothing downstream of raw emission exists in the committed code yet.
