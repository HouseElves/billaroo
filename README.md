...
Cancellation is the only lifecycle transition currently implemented. Recurring billing runs across the full simulation, and its invoices and invoice lines are emitted as raw `invoices.csv` and `invoice_lines.csv` files alongside the established artifacts, with manifest entries and record counts. The CLI summary reports the billing files and their row counts, and the canonical smoke gate verifies the billing artifacts exist and are internally coherent (manifest agreement, line-to-invoice integrity, exact invoice-total reconciliation). The rest of the downstream pipeline — payments, hidden truth, PostgreSQL load, dbt models, metric reconciliation — is not yet built.
...
- billing beyond recurring charges: payments, usage events, taxes, fees, credits, adjustments, proration
...
Invoices written: 204
Invoices file: build/raw/invoices.csv
Invoice lines written: 204
Invoice lines file: build/raw/invoice_lines.csv
...
A single check-in script runs every committed quality gate — compileall, pytest, 100% branch coverage, pylint, and a CLI smoke test against a temporary directory. The smoke step verifies that `lifecycle_events.csv` contains at least one data row and that the manifest's reported count for that file agrees with the CSV row count, and that the billing artifacts are coherent: `invoices.csv` and `invoice_lines.csv` exist and are non-empty, their manifest counts agree with the CSV row counts, every emitted invoice line references an emitted invoice, and each invoice total equals the exact sum of its lines. The smoke test never dirties `build/raw` or the working tree:
