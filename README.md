...
- `model/` builders — `build_account`, `build_subscriber`, `build_plan_subscription`, `build_feature_subscription`, `build_catalog`, `build_default_catalog`, and `build_invoice` / `build_invoice_line`, which construct individual deterministic, validated invoice and invoice-line records (the records only — no chargeability, totalling, or emission)
...
The `Invoice` and `InvoiceLine` record contracts and their `build_invoice` / `build_invoice_line` builders now exist, but Billaroo does not yet generate or emit invoices through its runnable simulation path: nothing decides which subscriptions are chargeable, calculates an account's bill, runs a billing action, accumulates invoices across the run, or writes invoice files. The hidden truth ledger, the rest of the billing and payments capability, the PostgreSQL loader, the dbt reconstruction layer, the validation comparison, and all non-cancellation lifecycle transitions are **planned architecture, not current capability**. The roadmap is recorded honestly in the design log; nothing downstream of raw emission exists in the committed code yet.
